...
impl<'a, 'b> Mul<&'a Vec3> for &'b Vec3 {
  ...
  fn mul(self, other: &'a Vec3) -> Self::Output {
    *self * *other
  }
}
...
impl<'a, 'b> Div<&'a Vec3> for &'b Vec3 {
  type Output = Vec3;

  fn div(self, other: &'a Vec3) -> Self::Output {
    *self / *other
  }
}
...
impl<'a> From<Vec3> for Cow<'a, Vec3> {
  fn from(item: Vec3) -> Self {
    Cow::Owned(item)
  }
}

impl<'a, 'b> From<&'b Vec3> for Cow<'a, Vec3>
where
  'b: 'a,
{
  fn from(item: &'b Vec3) -> Self {
    Cow::<'a>::Borrowed(item)
  }
}
